...
char ssid[] = "LC2";                       // Your WiFi credentials.
char pass[] = "lc@tiet1";
 ...
 void readGPS() {
  while (Serial.available() > 0) {
    if (gps.encode(Serial.read())) {
      Blynk.virtualWrite(V8, gps.location.lat(), 6); // Latitude
      Blynk.virtualWrite(V9, gps.location.lng(), 6); // Longitude
      Blynk.virtualWrite(V10, gps.speed.kmph()); // Speed in km/h
    }
  }
  }
